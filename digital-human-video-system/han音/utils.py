...
def load_checkpoint(checkpoint_path, model, optimizer=None, skip_optimizer=False):
    ...
    for k, v in state_dict.items():
        try:
            ...
            elif k in saved_state_dict and saved_state_dict[k].shape != v.shape:
                # Shape mismatch (e.g., embedding layer has more symbols/tones now)
                logger.warn(
                    f"Shape mismatch for {k}: checkpoint {saved_state_dict[k].shape} vs model {v.shape}, will load compatible portion"
                )
                if len(saved_state_dict[k].shape) == 2 and len(v.shape) == 2:
                    # 2D weight (e.g., Embedding): copy overlapping portion
                    min_dim0 = min(saved_state_dict[k].shape[0], v.shape[0])
                    min_dim1 = min(saved_state_dict[k].shape[1], v.shape[1])
                    with torch.no_grad():
                        v[:min_dim0, :min_dim1] = saved_state_dict[k][:min_dim0, :min_dim1]
                elif len(saved_state_dict[k].shape) == 1 and len(v.shape) == 1:
                    # 1D weight (e.g., bias): copy overlapping portion
                    min_dim = min(saved_state_dict[k].shape[0], v.shape[0])
                    with torch.no_grad():
                        v[:min_dim] = saved_state_dict[k][:min_dim]
                else:
                    logger.error(f"Unhandled shape mismatch for {k}: checkpoint {saved_state_dict[k].shape} vs model {v.shape}")
            else:
